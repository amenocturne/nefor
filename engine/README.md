...
The engine is session-blind: it owns no session ID, writes no on-disk log, and does not parse envelope bodies. Cross-session persistence, resumption, and impersonation are the responsibility of `lua/libs/sessions` (`libs.sessions`).
