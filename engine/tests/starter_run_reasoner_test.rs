//! Unit tests for `starter/reasoners/run.lua` and
//! `starter/reasoners/run-wrappers.lua`. Mirrors the harness pattern
//! in `starter_agentic_workflow_test.rs`: stub the `nefor.*` surface,
//! then load the Lua test driver at `starter/reasoners/run_test.lua`.
...
fn starter_run_reasoner_full() {
    ...
    let test_path = repo_root().join("tests/lua/reasoners/run_test.lua");
