...
- `lua/libs/agentic-loop/` — the lead's turn spawner **mechanism** (`libs.agentic-loop`): per user message it clones the config's turn-program (`starter/agentic-loop/lead-turn.mag`) and submits it to the mag kernel; owns canonical history + queueing. Native `/compact` materializes that history into a temporary provider chat, persists the opaque artifact at a transcript cutoff, and restores it into later ephemeral MAG chats; the full transcript remains the fallback across provider/model switches. `starter/agentic-loop/` keeps the config-owned turn-program data (`lead-turn.mag`) plus a re-export shim.
- `lua/libs/mag-workspace/` — MAG workspace management **mechanism** (`libs.mag-workspace`): per-session workspace seeding plus modification preview formatting; compilation itself lives in the mag plugin. `starter/mag/lib/` keeps the config-owned seed content (types, templates, tools, policies, patterns, prompts) that a workspace is seeded from.
...
- `starter/chat/` — the chat surface's **opinion layer**: `init.lua` (composition root — installs the require searchers and hands view/update to `tui.start`), `statusline.lua` (segments), `slash.lua` (command registry), `update.lua` (reducer + mode handlers). These `require("libs.chat.<m>")` for the mechanism. The pure state→view mechanism lives in `lua/libs/chat/` (see below).
...
- `lua/libs/lead-workflow/` — lead-workflow **mechanism**: the mag / mag-eval / write-review / graph-status / terminate-graph tool surface (`init.lua`), the one-off MAG-expression tool (`mag-eval.lua`), and the persona-prompt loader (`role.lua`). Config-agnostic — resolves prompt/data roots from `NEFOR_CONFIG_DIR`/`NEFOR_DATA_DIR`, never from file location, so the persona prompt stays config-owned at `<config>/prompts/lead.md`. `starter/lead-workflow/` holds thin re-export shims (`init.lua`, `role.lua`) so `starter/init.lua`'s `require("lead-workflow")` spawn site resolves unchanged; downstream configs `require("libs.lead-workflow")` directly.
...
- `lua/libs/read-only-tools/` — read-only investigation tools **mechanism** (`libs.read-only-tools`): the `list_dir`/`search_text`/`python-read`/`instructions`/`discover_instruction_files` handlers plus the tool-gate advertise/dispatch plumbing. `build{ extra_tools = { { schema, handler } } }` returns the actor spec, where a config-registered handler is `function(args, emit)` with `emit.ok(text)`/`emit.err(msg)` — the registration seam that lets a downstream config add typed tools (e.g. `mirror-projects`, `skill`) without forking. `starter/read-only-tools/` is a thin composition file that builds the base set with no extras.
- `lua/libs/tool-validator/` — tool-permission validator **mechanism** (`libs.tool-validator`): classifies gated invocations (bash through `da`, edit/write policy, read-only auto-approve) into approve/deny/popup. `build{ auto_approve_tools, bash_fastpaths }` returns the actor spec — `auto_approve_tools` names tools approved unconditionally, `bash_fastpaths` is a list of `function(command, read_only) -> bool` predicates that approve a bash command before `da` is consulted. Config-owned policy plugs in through those seams. `starter/tool-validator/` is a thin composition file that builds the base policy with no extras.
