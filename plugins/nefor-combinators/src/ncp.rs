//! NCP stdio transport for nefor-combinators.
//!
//! Mirrors the pattern used by `mock-plugin` and `mock-plugin`:
//!
        ...
        loop {
            match reader.next_line().await {
                Ok(Some(line)) => {
                    ...
                    let parsed = Envelope::parse_line(trimmed).map_err(CombinatorsError::from);
                ...
                Ok(None) => break,
                Err(e) => {
                    let _ = tx.send(Err(CombinatorsError::Io(e))).await;
                    break;
                }
            }
        }
...
/// message returns [`CombinatorsError::ReadyFailed`] carrying a code+message
...
pub async fn await_ready_ok(
    rx: &mut mpsc::Receiver<Result<Envelope, CombinatorsError>>,
) -> Result<String, CombinatorsError> {
    ...
    loop {
        let env = match rx.recv().await {
            ...
            None => return Err(CombinatorsError::ReadyClosed),
        };
        match env.body {
            ...
            Body::System(SystemBody::Error { code, message, .. }) => {
                return Err(CombinatorsError::ReadyFailed(format!(
                    "{code:?}: {message}"
                )));
            }
            other => {
...
mod tests {
    ...
    async fn await_ready_ok_accepts_ready_ok() {
        let (tx, mut rx) = mpsc::channel::<Result<Envelope, CombinatorsError>>(4);
    ...
    async fn await_ready_ok_detects_closed_stream() {
        let (_tx, mut rx) = mpsc::channel::<Result<Envelope, CombinatorsError>>(1);
        drop(_tx);
        let err = await_ready_ok(&mut rx).await.unwrap_err();
        assert!(matches!(err, CombinatorsError::ReadyClosed));
    }
}
