//! NCP stdio transport for generic-tool.
//!
//! Mirrors the pattern used by `generic-provider` — passive plugin with a
//! single startup `combinators.register` and an idle wait for shutdown.
...
pub fn spawn_stdin_reader(
    tx: mpsc::Sender<Result<Envelope, ToolError>>,
        ...
        loop {
            match reader.next_line().await {
                Ok(Some(line)) => {
                    ...
                    let parsed = Envelope::parse_line(trimmed).map_err(ToolError::from);
                ...
                Ok(None) => break,
                Err(e) => {
                    let _ = tx.send(Err(ToolError::Io(e))).await;
                    break;
                }
            }
        }
...
pub async fn await_ready_ok(
    rx: &mut mpsc::Receiver<Result<Envelope, ToolError>>,
    ...
    loop {
        let env = match rx.recv().await {
            ...
            None => return Err(ToolError::ReadyClosed),
        };
        match env.body {
            ...
            Body::System(SystemBody::Error { code, message, .. }) => {
                return Err(ToolError::ReadyFailed(format!("{code:?}: {message}")));
            }
            other => {
...
mod tests {
    ...
    async fn await_ready_ok_accepts_ready_ok() {
        let (tx, mut rx) = mpsc::channel::<Result<Envelope, ToolError>>(4);
    ...
    async fn await_ready_ok_detects_closed_stream() {
        let (_tx, mut rx) = mpsc::channel::<Result<Envelope, ToolError>>(1);
        drop(_tx);
        let err = await_ready_ok(&mut rx).await.unwrap_err();
        assert!(matches!(err, ToolError::ReadyClosed));
    }
}
