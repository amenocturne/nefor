...
-- consumer (tool-gate wrapper, etc.) can emit the right envelope
-- without baking the protocol shape into its own code.
--
-- Extracted from the old `lua/core-libs/graph/init.lua` during the
-- core/libs split (the orchestrator-template helper migrated to
-- `starter/agentic-loop/topology.lua` since it's starter-specific).
