...
use chatgpt_provider::responses::headers::{
    add_turn_headers, build_headers, CHATGPT_ACCOUNT_ID, ORIGINATOR, SESSION_ID, THREAD_ID,
...
fn adds_stable_session_routing_headers() {
    ...
    add_turn_headers(&mut headers, "session-42", "thread-42", Some("sticky-7"))
        .expect("turn headers");

    assert_eq!(headers.get(SESSION_ID).unwrap(), "session-42");
    assert_eq!(headers.get(THREAD_ID).unwrap(), "thread-42");
...
fn omits_turn_state_before_server_assigns_it() {
    let snap = snapshot_with_account();
    let mut headers = build_headers(&snap, "install-1", "nefor_cli_rs").expect("headers");
    add_turn_headers(&mut headers, "session-42", "thread-42", None).expect("turn headers");
