...
async fn run() -> Result<(), ProviderError> {
    ...
    let (in_tx, mut in_rx) = mpsc::channel::<Result<Envelope, ProviderError>>(ncp::CHANNEL_CAP);
...
async fn idle_until_shutdown(
    in_rx: &mut mpsc::Receiver<Result<Envelope, ProviderError>>,
...
async fn send_event(
    ...
    out_tx
        ...
        .await
        .map_err(|_| ProviderError::WriterClosed)
}

async fn send_ready(out_tx: &mpsc::Sender<PluginOutgoing>) -> Result<(), ProviderError> {
    out_tx
