...
local function validate_known_roles(node_specs)
  for _, spec in ipairs(node_specs or {}) do
    local role = type(spec) == "table" and spec.role or nil
    local deterministic = role == "accumulate" or role == "retry" or role == "bash_command"
    if type(role) == "string" and not deterministic
        and role_config(spec.role) == nil then
      return string.format(
        "dispatch-graph: node `%s` references unknown role `%s`",
        tostring(spec.id), spec.role)
    end
  end
  return nil
end
...
-- model? }` from `lead-workflow.role.AGENT_CONFIGS`. Unknown roles are
-- rejected here: the lead workflow is a role-keyed contract, so falling
-- through to a generic agent would bypass role prompts and writer gates.
local function build_graph_spec(node_specs, terminal_id)
  ...
  local role_err = validate_known_roles(node_specs)
  if role_err then return nil, role_err end
  ...
  for _, spec in ipairs(node_specs) do
    ...
    elseif spec.role == "bash_command" then
      ...
      node = {
        ...
        reasoner = "agent",
        args     = agent_args,
        -- Carries the lead's role label (explorer / worker / reviewer /
        -- docs / critic) through to the chat surface so the graph_result block can
...
-- Approval gate: write-capable roles (worker, docs, and any role explicitly
-- tagged `read_only = false`) require an approved plan before the lead can
-- dispatch them. Read-only investigation (explorer, reviewer, critic) is
-- always allowed.
--
...
local function writer_denial_message(prefix, plan_state, writers)
  return string.format(
    ...
    "roles (explorer/reviewer/critic).",
...
local function gate_against_unapproved_plan(node_specs)
  ...
  local role_err = validate_known_roles(node_specs)
  if role_err then return role_err end
              ...
              properties = {
                ...
                role         = {
                  ...
                  description = "Sub-agent role. Drives the system prompt, model, and " ..
                    ...
                    "(read-only investigation), worker (writes code; requires an " ..
                    "approved plan), reviewer (read-only review), docs " ..
                    "(documentation/Jira/Confluence; requires an approved plan), " ..
                    "and critic (read-only critique). Deterministic roles: " ..
                    "accumulate, retry, bash_command.",
                },
                args         = {
                  type        = "object",
