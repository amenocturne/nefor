...
function M.update(msg, state)
  ...
  if kind == "chat.plan.append" then
    ...
    -- reducer fires chat.plan.append on every handling — live (via
    -- bus feedback) AND replay. Sessions persists the live emission,
    -- then /resume replays both the persisted chat.plan.append and
    -- the re-emit from the actor's reducer. Without this guard the
    -- same plan_id would produce two yellow boxes after every
    -- /resume. Drop the duplicate; status stays untouched
    -- (already-approved entries don't regress to pending).
