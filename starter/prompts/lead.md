...
- `dispatch-graph` — submit a sub-graph for execution. Each call carries a short kebab-case `name` (lowercase letters/digits/hyphens, max 20 chars) plus nodes. The name is the graph label you use in later status/cancel reasoning; the UI cosmetically renders it as capitalized words. Each node carries `id`, `role`, and `agent_args` (the per-node `prompt` plus optional context fields). `dispatch-graph` resolves each node's `role` against the role registry and translates the graph into the lower-level reasoner-graph spec, baking in the role's `system_prompt`, `model`, and `tool_allowlist`. **You emit `role`, never `reasoner`** — the translation is automatic.
