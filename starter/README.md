...
- `tool-validator/` — a thin composition file. The permission-classification mechanism (bash-via-`da`, edit/write policy, allowlist-derived read-only handling) ships at `lua/libs/tool-validator/`; this file parses the canonical `mag/lib/nefor/toolsets.json` manifest and supplies its read-only inventory to the mechanism.
- `mag/lib/nefor/toolsets.json` — the single config-owned neutral data manifest for starter tool inventories. `mag/lib/nefor/toolsets.mag` exposes it as `nefor.actors.read-only-tools` and `nefor.actors.general-tools` through MAG's typed JSON parser, while tool-validator uses the engine's JSON binding. Agent `:tools` remains the sole capability boundary; explicit custom lists are supported, and tool-gate enforces membership before policy or forwarding.
