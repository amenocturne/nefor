...
- `lead-workflow/` — re-export shims for the lead-workflow mechanism, which now lives in the shared lua tree at `lua/libs/lead-workflow/` (actor plan/approval state, kernel-run tracking, mag-eval tool, and prompt composition). The shared base and root overlay stay config-owned at `prompts/base.md` and `prompts/lead.md`.
