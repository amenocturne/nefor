...
local function on_tool_result(body)
  ...
  local reasoner = meta.reasoner or "bash_command"
  firing_meta[firing_id] = nil

  if type(body.error) == "string" and #body.error > 0 then
    emit_as(reasoner, nil, { kind = "tool.result", id = firing_id, error = body.error })
    return
  end
  if type(body.output) ~= "string" then
    emit_as(reasoner, nil, { kind = "tool.result", id = firing_id, error = "bash_command reasoner: bash returned non-string output" })
    return
  end
