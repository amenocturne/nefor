//! Unit tests for `starter/reasoners/agent.lua`. Mirrors the harness
//! pattern in `starter_agentic_workflow_test.rs`: stub the `nefor.*`
//! surface, then load the Lua test driver at
//! `starter/reasoners/agent_test.lua`.
...
#[test]
fn starter_agent_reasoner_full() {
    let lua = Lua::new();
    install_stub_nefor(&lua).expect("install nefor stub");
    set_package_path(&lua).expect("set package.path");

    let test_path = repo_root().join("tests/lua/reasoners/agent_test.lua");
...
fn install_stub_nefor(lua: &Lua) -> mlua::Result<()> {
    ...
    // Bus subscription stub — same shape as starter_agentic_workflow_test.
    ...
    // engine — captures send calls into _engine_calls.
