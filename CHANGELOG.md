...
- Added composition seams for providers, tools, read-only capabilities, validation policy, sessions, lead workflows, and CLI surfaces; clarified the engine/plugin/library/config ownership boundaries and hardened source, stable, and nightly distributions with immutable installation identities and release-bundle validation.
