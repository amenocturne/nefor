...
Structured message content remains the canonical provider/model value. A producer that also knows the corresponding human-authored text may record it as validated `display_text` metadata on the message start; conversation-manager preserves both forms, surfaces prefer the explicit presentation, and generic structured rendering remains the fallback when no presentation was supplied.
