...
Omit session IDs to copy every provenance-aware source session. Source JSONL/event bytes and the complete session tree, including MAG artifacts, are copied without interpretation. The destination generation ID is appended to copied metadata, but Nefor makes no claim that the destination can understand the payload. Any destination collision aborts that session rather than overwriting it. Per-session interprocess locks and same-filesystem staging/rename prevent readers from observing partial session metadata or trees.
