...
- Read every changed file completely with `read_file`; inspect diffs and search
  ...
  - `(nefor.shell.script "diff" (as nefor.contracts.ShellScriptParams {:script "git diff | head -200" :cwd "." :timeout (nefor.contracts.no-timeout)}))`
  - `(nefor.shell.script "search" (as nefor.contracts.ShellScriptParams {:script "rg -n 'caller_of_changed_fn' src/" :cwd "." :timeout (nefor.contracts.no-timeout)}))`
